...
parent_folder = '649343a7e669c06230ccb9e5'
destination_folder_name = chromosome_interested + '_exome'
...
exome_vcf_files = [file for file in all_files_in_folder if file.name.endswith('.vcf.gz')]

# Inputs
inputs = {}
inputs['input_vcf'] = exome_vcf_files
inputs['output_basename'] = chromosome_interested +  '_exome'  # Add the output_basename input here
inputs['tool_name'] = 'vep_105'  # Add the output_basename input here


# Specify that one task should be created per file (i.e. batch tasks by file)
batch_by = {'type': 'item'}

# Specify that the batch input is 'input_vcf'
batch_input = 'input_vcf'  # Change this to match the app's input name
